#!/bin/bash
# DONT_USE_CELLS sweep: DU=def (platform default) | all (only SDF*/ICG* excluded)
...
T=$1; P=$2; DU=$3; V=s4u40_${T}_P${P}_DU${DU}
...
make DESIGN_CONFIG=designs/cryo7/gcd/config.mk CRYO_TEMP=$T FLOW_VARIANT=$V \
  ...
  ENABLE_PLACE_REPAIR_TIMING=1 SKIP_LAST_GASP=0 OPT_POST_GRT_WNS=0 CORE_UTILIZATION=40 "${EXTRA[@]}" \
